...
var drawHorizontalLine = function(that, canvasWidth, sortedXValues) {
  ...
  if(drawRight > that.width - that.startX) {
    that.x, that.y, that.width += 2, that.height
  }
  if(drawLeft <= that.startX-that.x && drawRight <= (that.width - that.startX)) {
...
// drawRight > that.width - that.startX
  ...
  if(that.startY - that.y >= drawAbove && (drawBelow <= (that.height - (that.startY - that.y)))) {
    that.building_wall = false;
  }
};

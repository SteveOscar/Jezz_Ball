...
var drawHorizontalLine = function(that, canvasWidth, sortedXValues) {
  ...
  // while(that.x >= wallToLeft) {
  //   that.context.moveTo(that.startX, that.y)
  //   that.context.lineTo(wallToLeft, that.y)
  //   that.context.lineTo(wallToRight, that.y)
  //   that.contest.stroke();
  //     // that.x -= 2, that.y, that.width += 2, that.height;
  // }

  // while(right <= wallToRight){
    that.context.moveTo(that.startX, that.y)
    that.context.lineTo(wallToLeft, that.y)
    that.context.lineTo(wallToRight, that.y)
    that.context.stroke();

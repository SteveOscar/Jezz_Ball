...
Ball.prototype.move = function(canvasWidth, canvasHeight, walls) {
  ...
  if(rightSideOfBall > canvasWidth || leftSideOfBall < 0) {
	...
	if( walls.length > 0) {
		...
		if (hitsBuildingHorizontalWall(this) && yValuesOfWall.includes(bottomOfBall)) {
			this.lose = true;
			console.log("Bottom of ball " + bottomOfBall);
		}
		if (hitsBuildingHorizontalWall(this) && yValuesOfWall.includes(topOfBall)) {
			this.lose = true;
			console.log("You SHOULD LOsE NOW");
		}

		if ((	yValuesOfWall.includes(bottomOfBall) ||
					yValuesOfWall.includes(bottomOfBall) ||
					yValuesOfWall.includes(topOfBall) ||
					yValuesOfWall.includes(topOfBall)) &&
					walls[walls.length - 1].building_wall === false) {
		 this.vy = -this.vy;
		 }
	}
	if(walls.length > 0){
		if (hitsBuildingVerticalWall(this) && xValuesOfWall.includes(rightSideOfBall)) {
			this.lose = true;
			console.log("You SHOULD LOsE NOW!!!!!!!!!!!!!!!!!!!!!!!");
		}
		if (hitsBuildingVerticalWall(this) && xValuesOfWall.includes(leftSideOfBall)) {
			this.lose = true;
			console.log("You SHOULD LOsE NOW!!!!!!!!!!!!!!!!!!!!!!!!!!");
		}

		if ((xValuesOfWall.includes(rightSideOfBall) ||
				xValuesOfWall.includes(rightSideOfBall) ||

...
Ball.prototype.move = function(canvasWidth, canvasHeight, walls) {
	...
	// var WallHelper.xValuesOfWalls(walls = walls.map(function(wall){
	// 	return wall.x, (wall.x + 1);
	// });

	// var WallHelper.yValuesOfWalls(walls )= walls.map(function(wall){
	...
	if( walls.length > 0) {
		if (hitsBuildingHorizontalWall(this) && ballHitWall(BallEdges.bottomOfBall(this), WallHelper.yValuesOfWalls(walls)) && wallHitBelow.building_wall === true) {
			this.lose = true;
		}
		if (hitsBuildingHorizontalWall(this) && ballHitWall(BallEdges.topOfBall(this), WallHelper.yValuesOfWalls(walls)) && wallHitAbove.building_wall === true) {
			this.lose = true;
		}
		if (hitsBuildingVerticalWall(this) && ballHitWall(BallEdges.rightSideOfBall(this), WallHelper.xValuesOfWalls(walls)) && wallHitToRight.building_wall === true) {
			this.lose = true;
		}
		if (hitsBuildingVerticalWall(this) && ballHitWall(BallEdges.leftSideOfBall(this), WallHelper.xValuesOfWalls(walls)) && wallHitToLeft.building_wall === true) {
			this.lose = true;
		}

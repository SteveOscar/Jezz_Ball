...
Ball.prototype.move = function(canvasWidth, canvasHeight, walls) {
  ...
  if(rightSideOfBall > canvasWidth || leftSideOfBall < 0) {
	...
	if( walls.length > 0) {
		if (yValuesOfWall.includes(bottomOfBall) || yValuesOfWall.includes(topOfBall)) {
		 this.vy = -this.vy;
		 }
	}
	if(walls.length > 0){
		if (xValuesOfWall.includes(rightSideOfBall) || xValuesOfWall.includes(leftSideOfBall)) {
		this.vx = -this.vx;
		}
	}
